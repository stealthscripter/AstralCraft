...
function GameLayout() {
  ...
  const [selectedComputerFingers, setSelectedComputerFingers] = useState<string[]>([]);
  ...
  const computerFinger = useSelector((state: RootState) => state.gameState.computerFinger);
  const playerFinger = useSelector((state: RootState) => state.gameState.playerFinger);
  const userPick = useSelector((state: RootState) => state.picksVariable.userPicks);
  const computerPick = useSelector((state: RootState) => state.picksVariable.computerPicks);
      ...
      {isStarted ? (
        <>
          <section className="col-span-7 ">
              <h1 className="text-sm text-center">red box variable you choose and blue box is belong to computer ( its not matrix )</h1>
          </section>
          ...
          <section className="flex justify-center">
            ...
            >
              {isProcessing ? "Processing..." : "Ready To Play"} {/* Show loading text */}
            </button>
          </section>
          <section className="col-span-3">
        ...
        <>
         <section className="col-span-7 ">
              <h1 className="text-sm text-center">you can choose the thrown finger number by click fingers and toggle it </h1>
          </section>
          <section className="col-span-3">
            <PlayerCore
          ...
          <section className="flex justify-center">
            <button
              className="border border-amber-700 col-start-3 col-span-3 py-3 cursor-pointer mt-10 px-3 leading-8"
